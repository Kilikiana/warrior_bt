...
class BullFlagDetector:
    ...
    def __init__(self, strict: bool = True):
        ...
        # Pullback length: allow 1–5 bars (ideal 2–3)
        self.max_pullback_candles = 5
    ...
    def analyze_pullback(self, df: pd.DataFrame, flagpole_high_idx: int,
        ...
        pull_avg_vol = df['volume'].iloc[flagpole_high_idx+1:end_for_pb+1].mean() if end_for_pb > flagpole_high_idx else 0.0
    ...
    def detect_entry_signal(self, df: pd.DataFrame, flagpole_high: float,
        ...
        """
        last_red_high = pullback_analysis.get('last_red_high')
        red_len = pullback_analysis.get('red_candles', 0)
        if last_red_high is None:
            return False, None
        if red_len < self.min_pullback_candles or red_len > self.max_pullback_candles:
            return False, None
        cur = df.iloc[-1]
        # Trigger on high crossing the pullback high; don't require green close
    ...
    def detect_bull_flag(self, df: pd.DataFrame, symbol: str = "", 
        ...
        if validation != BullFlagValidation.VALID:
