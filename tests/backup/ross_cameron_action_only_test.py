...
# Try to import from package structure; else fall back to local modules/stubs
try:
    from position_management.position_sizer import PositionSizer
except Exception:
    class PositionSizer:
        def size_position(self, account_balance: float, entry_price: float, stop_loss: float, risk_pct: float=0.02) -> int:
            risk_amount = account_balance * risk_pct
            rps = max(1e-6, abs(entry_price - stop_loss))
            return max(0, int(risk_amount // rps))

try:
    from position_management.position_tracker import PositionTracker, RiskLimitViolation
except Exception:
    class RiskLimitViolation(Exception): pass
    class PositionTracker:
        def __init__(self, account_balance: float, max_positions: int = 4, daily_risk_percentage: float = 0.05):
        ...
        def daily_risk_budget(self):
            return self.account_balance * self.daily_risk_percentage
        def can_open_new_position(self):
            return len(self.positions) < self.max_positions
        def register_position(self, symbol, shares, entry_price, stop_loss):
            self.positions.append({'symbol': symbol, 'shares': shares, 'entry': entry_price, 'stop': stop_loss})
            return type('Obj', (), {'shares': shares})
        def get_position_summary(self):
            return {'active_positions_count': len(self.positions), 'max_positions_limit': self.max_positions, 'daily_risk_budget': self.daily_risk_budget}

try:
    from tech_analysis.patterns.pattern_monitor import ActionAlertPatternMonitor
except Exception:
    from pattern_monitor import ActionAlertPatternMonitor

try:
    from tech_analysis.patterns.bull_flag_pattern import BullFlagDetector
except Exception:
    from bull_flag_pattern import BullFlagDetector
...
class RossCameronTradingSystem:
    ...
    def run_integration_test(self, scan_file: str):
        ...
        """
        logger.info("="*80)
        logger.info("ROSS CAMERON ACTION-ONLY TEST (NO GAPPERS)")
        logger.info("="*80)
        logger.info("ACTION-ONLY approach: Trade ALL ACTION alerts without gappers requirement")
        logger.info("Every ACTION alert will be considered for trading")
        ...
        logger.info("Note: This version trades ALL ACTION alerts without gappers filtering")
        ...
        for alert in sorted(action_alerts, key=lambda x: x['time']):
            ...
            # ACTION-ONLY: Trade ALL ACTION alerts without gappers requirement
    ...
    def print_final_summary(self, processed_alerts: int, successful_entries: int, priority_alerts_count: int = 0):
        ...
        logger.info(f"ACTION alerts processed: {priority_alerts_count}/{processed_alerts}")
        logger.info("Note: All ACTION alerts were considered for trading (no gappers filtering)")
